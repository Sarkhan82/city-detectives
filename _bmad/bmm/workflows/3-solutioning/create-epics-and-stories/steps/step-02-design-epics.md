...
5. **Parallelization**: **depends_on** (epic IDs that must be done before this epic, e.g. "Epic 1"), **parallelizable_with** (epic IDs that can be developed in parallel by another agent, e.g. "Epic 3, Epic 4"), **lane** (optional: lane/track ID for agent assignment, e.g. "A", "B")
...
**Depends on:** Epic 1 (or "none")
**Parallelizable with:** Epic 3, Epic 4 (epics that can be developed in parallel by another agent)
**Lane:** A (optional: for assigning to agent lane A/B/C)
...
4. Replace {{parallelization_matrix}} with a **Parallelization & Agent Lanes** section: table or list showing for each epic: **Epic** | **Depends on** | **Parallelizable with** | **Lane** (optional). This allows multiple agents to work in parallel on independent epics.
